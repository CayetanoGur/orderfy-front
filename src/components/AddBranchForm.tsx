...
const AddBranchForm: React.FC = () => {
  ...
  const validateForm = () => {
    ...
    if (formData.ubication.length > 200) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`http://127.0.0.1:8000/database/${restaurantSlug}/add_branch/`, {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData || 'Failed to add branch');
      }

      // Navigate back to the branches list
      navigate(`/dashboard/restaurant/${restaurantSlug}`);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'An error occurred while adding the branch');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="max-w-2xl mx-auto bg-white rounded-lg shadow-md p-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            <label htmlFor="ubication" className="block text-sm font-medium text-gray-700">
              Location
            </label>
            <input
              type="text"
          ...
          <div>
            ...
            <input
              ...
              required
              className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
              placeholder="Enter branch slug (e.g., branch-name)"
            />
